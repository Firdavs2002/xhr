...
router.use(log), router.use(cors);
{
  ...
  router.register(
    ...
    "/api/hw29/posts",
    (t, s) => {
      s.setHeader("Content-Type", "application/json"), s.end(JSON.stringify(e));
    },
    slow
  );
}
      ...
      if (++t % 3 == 0)
        return (
          ...
          void r.end(
            JSON.stringify({
              message:
                "Ð¡ÐµÑ€Ð²ÐµÑ€ Ð²Ñ€ÐµÐ¼ÐµÐ½Ð½Ð¾ Ð½ÐµÐ´Ð¾ÑÑ‚ÑƒÐ¿ÐµÐ½, Ð¿Ð¾Ð¿Ñ€Ð¾Ð±ÑƒÐ¹Ñ‚Ðµ Ð¿Ð¾Ð²Ñ‚Ð¾Ñ€Ð¸Ñ‚ÑŒ Ð²Ð°Ñˆ Ð·Ð°Ð¿Ñ€Ð¾Ñ Ð¿Ð¾Ð·Ð¶Ðµ",
            })
          )
        );
    ...
    router.register(
      ...
      (e, s) => {
        ...
        if (Number.isNaN(r))
          return (
            (s.statusCode = 400),
            void s.end(JSON.stringify({ error: "bad id" }))
          );
  ...
  router.register(
    ...
    router.register(
      ...
      (e, s) => {
        ...
        (t = t.map((e) => (e.id !== r ? e : { ...e, likes: e.likes + 1 }))),
          (s.statusCode = 204),
          s.end();
      },
      slow
    ...
    router.register(
      ...
      (e, t) => {
        ...
        if (Number.isNaN(r))
          return (
            (t.statusCode = 400),
            void t.end(JSON.stringify({ error: "bad id" }))
          );
        (s = s.filter((e) => e.id !== r)), (t.statusCode = 204), t.end();
      },
      slow
    ),
    router.register(
      ...
      (e, t) => {
        ...
        (s = s.map((e) => (e.id !== r ? e : { ...e, likes: e.likes + 1 }))),
          (t.statusCode = 204),
          t.end();
      },
      slow
    ...
    router.register(
      ...
      (e, r) => {
        ...
        if (Number.isNaN(i))
          return (
            (r.statusCode = 400),
            void r.end(JSON.stringify({ error: "bad id" }))
          );
        const o = e.body;
        (o.id = t++),
          (s = s.map((e) =>
  ...
  router.register(
    ...
    router.register(
      ...
      (t, r) => {
        const i = t.body;
        0 === i.id
          ? ((i.id = e++), (i.likes = 0), (i.comments = []), (s = [i, ...s]))
          : (s = s.map((e) =>
              e.id !== i.id ? e : { ...e, author: i.author, text: i.text }
            )),
          r.setHeader("Content-Type", "application/json"),
          r.end(JSON.stringify(i));
      },
...
}
{
  ...
  router.register("GET", "/api/hw36/posts", (e, t) => {
    ...
    router.register("DELETE", /^\/api\/hw36\/posts\/(?<id>\d+)$/, (e, t) => {
      const r = Number(e.matches.id);
      if (Number.isNaN(r))
        return (
          (t.statusCode = 400), void t.end(JSON.stringify({ error: "bad id" }))
        );
      (s = s.filter((e) => e.id !== r)), (t.statusCode = 204), t.end();
    }),
    router.register(
